...
class RotateButton: SKSpriteNode, StationaryButtonProtocol {
    ...
    var asset: UIImage? {
        didSet {
            asset!.withTintColor(.white)
            let data = asset!.pngData()
            asset = UIImage(data: data!)
        }
    }
    ...
    required init(systemName: String, anchorPoint: CGPoint, size: CGSize = CGSize(width: 50, height: 50), position: CGPoint, zPosition: CGFloat = 0, rotationDirection: String, spaceship: SKSpriteNode) { //"left" or "right"
        self.rotationDirection = rotationDirection
        self.asset = UIImage(systemName: systemName)
    ...
    required init(named: String, anchorPoint: CGPoint, size: CGSize = CGSize(width: 50, height: 50), position: CGPoint, zPosition: CGFloat = 0, rotationDirection: String, spaceship: SKSpriteNode) {
        self.rotationDirection = rotationDirection
        self.asset = UIImage(named: named)
        let texture = SKTexture(image: self.asset!)
        self.spaceship = spaceship
        super.init(texture: texture!)
        self.size = size
